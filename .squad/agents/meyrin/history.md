...
- 2026-06-12T14:14:36.351-04:00 — The Deploy API failure was not a C# syntax regression in the accept-state work: ACR's Docker publish omitted the repo-root `samples/` folder, so `CallCenterTranscription.Ai.csproj` could not embed `agent-assist-demo-scripts.v1.json` / trigger expectations and `dotnet publish` failed with CS1566 inside the container only. Any containerized API build must carry repo-level sample assets until those resources are moved under `src/`.
