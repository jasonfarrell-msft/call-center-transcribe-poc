---
## Active Decisions
- **2026-06-12 | Jason (Directive)** — Use only English for the demo.
- **2026-06-12 | Jason (Directive)** — Assume the demo can be run using our Azure resources.
- **2026-06-12 | Lunamaria** — Header softphone owns Accept visibility for answerable pending calls while the lower transcript badge stays scoped to speech-service status.
- **2026-06-12 | Lacus** — Demo sentiment uses shorter customer windows, rep resolution cues, translated-script scoring, and a resolved end-state for successful save flows.
- **2026-06-12 | Dyakka** — ACS answer path now waits for `CallConnected` before rep invite, resolves callback/media URIs from `Acs:PublicBaseUrl` with path-prefix support, and records redacted wiring diagnostics for wrong-number repros.
- **2026-06-12 | Meyrin** — Live pending/Accept is exposed only after a real ACS rep invite exists (`CallConnected` + `AddParticipant`), mock/demo mode never advertises ACS Accept, and stale callbacks are scoped to the active call ID.
...
# Lunamaria — Early Accept UI + lower badge scope

- **Date:** 2026-06-12
- **Status:** Implemented
- **Source:** `.squad/decisions/inbox/lunamaria-accept-ui-badges.md`

## Decision
- Treat the **header softphone bar** as the source of truth for rep call actions (`Accept`, `Decline`, in-call controls).
- Treat the **lower transcript badge** as the source of truth for **speech-service connectivity/status only**.
- On `stream.callPending`, show the header Accept affordance immediately, even if the browser has not yet received the local ACS `incomingCall` object.

## Why
Demo feedback showed that tying the visible Accept affordance too closely to the local ACS invite makes the rep feel late to the call. The SignalR pending event arrives earlier and is the right UI trigger for "a call is incoming." At the same time, the transcript badge was overloading call-state and infrastructure-state, which made the screen harder to read.

## Implementation notes
- `live-transcript.js` now writes `data-live-call-state` / `data-live-call-id` on the console root and dispatches `rep.callPending` / `rep.callAccepted` events.
- `rep-phone.js` listens for `rep.callPending` so the header bar can reveal Accept immediately; Accept stays disabled until the ACS SDK delivers the actual `incomingCall` handle.
- Decline remains available during this early pending window and still posts `/rep/hangup` so the PSTN leg is torn down if the rep declines before local audio fully arrives.
- The lower badge text now reflects speech-service states like offline / connected / reconnecting / transcribing, not pending-call lifecycle.

## Contract note for Meyrin
Today `/api/calls/active` only returns `{ callId }`, which is enough to tell the UI that **a call exists** but not enough to distinguish **pending** vs **already accepted** after reload/reconnect. The frontend now safely resyncs into **pending** first and waits for stronger evidence (`stream.callAccepted` or transcript replay) before promoting the UI.

### Requested follow-up
Please expose an authoritative live call-phase contract (for example `pending | accepted | ended`, ideally with rep-accepted state) on either:
- `/api/calls/active`, or
- `/api/session/current-state`

That would remove the remaining reload/reconnect ambiguity for the softphone header.

# Meyrin — Early Accept state contract

- **Date:** 2026-06-12
- **Status:** Implemented
- **Source:** `.squad/decisions/inbox/meyrin-early-accept-state.md`
- **Superseded by:** 2026-06-12 accept-answer invite gating decisions recorded below from Dyakka, Meyrin, Athrun, and Yzak.

- **Decision:** Treat backend lifecycle state as the earliest reliable source for Rep Accept availability.

## Decision
- `PipelineCurrentStateStore` is now the authority for live call bootstrap state.
- In `AudioSource:Mode=Acs`, the store starts `idle`, switches to `pending` immediately after `AnswerCallAsync`, and switches to `accepted` on `AddParticipantSucceeded`.
- `/api/session/current`, `/api/session/current-state`, and `/api/calls/active` all read from that authoritative state so late-joining browsers can discover a pending call before any customer utterance.
- SignalR replay now emits lifecycle events before transcript replay and suppresses transcript replay entirely while the snapshot is still `pending`.
- Mock/scripted mode remains authoritative for deterministic history, and live ACS callbacks are ignored when `AudioSource:Mode` is not `Acs`.

## Consequences
- The web app can bootstrap an incoming/pending Accept offer without waiting for transcript/customer speech.
- Live replay semantics are now honest: lifecycle-only until real buffered history exists.
- Future frontend work can bind directly to call state from REST/bootstrap and existing lifecycle stream names.

# Lacus — Demo sentiment tuning

- **Date:** 2026-06-12
- **Status:** Implemented
- **Source:** `.squad/decisions/inbox/lacus-demo-sentiment-tuning.md`

- **Owner:** Lacus

## Decision
Use a deterministic conversation tracker for demo sentiment that combines:
1. a short rolling window of the two most recent customer sentiment signals,
2. explicit rep resolution cues (delivery/payment-plan/renewal/auto-delivery commitments) only when the rep speaker is clearly attributed, and
3. explicit customer acceptance cues to allow the final state to land on `resolved`.

## Why
The prior sentiment behavior stayed negative too long because early complaint language dominated the full conversation, and scripted bilingual turns could miss scoring entirely. The demo needs visible mid-call movement and a final state that reflects a successful save/upsell outcome.

## Consequences
- Demo sentiment updates earlier, including before the final customer close when the rep makes a concrete recovery offer.
- Successful scripts (especially low-tank → auto-delivery conversion) finish `resolved` instead of merely less negative.
- Scripted non-English complaint turns use the deterministic English override for scoring when present.
- Live ACS remains limited by the known rep-first post-accept diarization ambiguity; do not treat this heuristic as production-grade attribution.

# Yzak — Demo feedback regression gate

- **Date:** 2026-06-12
- **Status:** APPROVE
- **Source:** `.squad/decisions/inbox/yzak-demo-feedback-regressions.md`

- **Owner:** Yzak

## Decision
Approve the resumed demo-feedback fixes for:
1. early pending/Accept availability before transcript, with the lower badge reserved for speech-service status only; and
2. low-tank auto-delivery sentiment recovery ending `resolved` instead of staying negative.

## Why
- `AcsEndpoints` now marks the call pending immediately after answer, `PipelineCurrentStateStore` exposes `acceptAvailable` before transcript, and `PipelineReplayPublisher` withholds transcript replay while the call is still pending.
- `live-transcript.js` keeps the lower badge on speech-service connectivity (`Speech services connected`) while `rep-phone.js` owns the top-bar Accept/Decline state.
- The low-tank scripted sentiment path now shows visible recovery before the close: turn 4 publishes an `improving` event, turn 6 is already `positive`, and the final customer acceptance resolves the summary.

## Evidence
- Added `ApiHost_LiveMode_ExposesAcceptedCallStateWithoutReopeningAccept`.
- Tightened `Feed_LowTankConversion_UpdatesSentimentBeforeFinalAcceptanceAndEndsResolved` to assert:
  - turn 4 sentiment trend is `improving`,
  - turn 6 sentiment label is `positive`,
  - final summary is `resolved`.
- Validation passed:
  - Focused regression suite: 7/7 green.
  - Full suite: `dotnet test CallCenterTranscription.sln --nologo --no-restore` → 123 total, 120 passed, 3 skipped, 0 failed.

## Non-blocking note
An independent second review re-raised the known live ACS limitation where a rep-first post-accept utterance can still confuse diarization if the customer was silent before accept. This is already documented as a pre-existing demo-scope limitation and is not a regression introduced by the pending/sentiment fixes reviewed here.

# Athrun — Accept resync revision
...
## Decision
- `live-transcript.js` resync must reopen the pending/Accept UI only when `/api/calls/active` reports `acceptAvailable: true` for the active call.
- Accepted-call recovery must fail closed unless `/api/session/current-state` confirms the same `callId`, a non-mock snapshot, and an accepted/live call state.
- `rep-phone.js` remains responsible for enabling the Accept button only after the browser has the real ACS `incomingCall` object.

## Why
- The rejected revision treated any active `callId` as enough to restore pending/Accept, which could reopen the softphone for accepted, mock, or otherwise non-answerable snapshots after reload/reconnect.
- The backend now exposes enough contract detail (`acceptAvailable`, `repAccepted`, current-state metadata) to distinguish an answerable pending call from an already accepted or mock-only state.
- We still need the earlier UX win where a genuinely answerable call shows quickly, without waiting for transcript replay.

## Consequences
- Reload/reconnect only restores pending when the call is truly answerable.
- Accepted live calls can still recover by resubscribing to the call group, but only after the current-state snapshot agrees with the active-call snapshot.
- Mock/live mismatches no longer light up the rep Accept affordance during resync.

# Dyakka — ACS inbound answer regression / wrong-number symptom

- **Date:** 2026-06-12
- **Status:** Implemented + operator checks required
- **Owner:** Dyakka

## Decision
Keep the UI's early `pending` state, but do **not** invite the rep leg until ACS sends `CallConnected` for the answered PSTN call. `/api/rep/register` must also respect that gate.

## Why
The latest latency optimization invited the rep too early in the post-`AnswerCallAsync` window. That made the softphone appear faster, but it could issue `AddParticipant` before the inbound PSTN call was fully connected, which matches the field symptom: Accept UI shows up, yet the caller hears a failed/wrong-number outcome and the service never truly answers.

## Code consequences
- `AcsEndpoints` now emits only `stream.callPending` immediately after `AnswerCallAsync`.
- `AddParticipant` is deferred until `CallConnected` (or a later `/rep/register` after `CallConnected`).
- `ActiveCallStore` now tracks `CallConnected` explicitly and resets/publishes its flags in race-safe order.
- ACS callback/media URLs now support `Acs:PublicBaseUrl`, preserve path prefixes, and log redacted identifiers plus resolved callback/media targets.

## Operator verification steps (Azure-resource demo)
1. Confirm the ACS Event Grid subscription points to `https://<public-api-host>/api/events/acs/incoming-call` and is delivering `Microsoft.Communication.IncomingCall`.
2. If the API sits behind a custom domain, Front Door, reverse proxy, or path prefix, set `Acs__PublicBaseUrl=https://<public-api-host>[/path-base]` so answer-time callback/media URIs match the externally reachable host.
3. Place a test PSTN call and verify logs show:
   - `IncomingCall received ... answering via callback https://.../api/events/acs/callbacks and media stream wss://.../api/calls/media-stream`
   - `ACS call answered; callId=...`
   - `ACS CallConnected for call ...`
   - only then `AddParticipant invited rep ...`
4. If step 3 never reaches `CallConnected`, inspect ACS callback reachability and webhook auth/routing before blaming the softphone.
5. If `IncomingCall` never arrives, re-check the phone number is attached to the same ACS resource that owns the Event Grid subscription.

## Fail-safe diagnostics to watch
- Warning if callback/media base resolves to `localhost`/non-public host.
- `IncomingCall received but CallAutomationClient is not registered` → `Acs:Endpoint`/identity/RBAC issue.
- `Failed to answer ACS incoming call` → answer call failure; inspect ACS exception + resource wiring.
- Missing `CallConnected` after answer success → callback URI or Azure routing issue.

# Meyrin — Accept answer state must follow a real ACS invite
...
## Decision
- The backend only exposes live `pending` / Accept availability after a real ACS rep invitation exists: `CallConnected` must arrive and `AddParticipant` must successfully issue the rep invite.
- `AnswerCallAsync` alone is no longer enough to publish pending state.
- Mock/scripted mode never exposes ACS Accept availability, even if code paths attempt to mark pending.
- ACS callback-driven state mutations (`connected`, `accepted`, teardown) are call-id scoped so stale events from an old call cannot mutate a newer live call.

## Why
The prior fast-path surfaced a pending Accept UI before the rep had an answerable ACS incoming-call leg. That created a synthetic state where the UI looked actionable but no real invite existed yet. In sequential live demos, stale ACS callbacks were also capable of mutating the next call's state.

## Consequences
- The rep only sees backend-driven Accept when ACS has created a real invite the browser can answer.
- Mock/demo playback can no longer accidentally advertise live ACS acceptability.
- Sequential live calls are safer because stale callback completions no-op instead of reopening invites or tearing down the next call.

# Yzak — Accept answerability regression gate
...
## Decision
Reject the current accept-latency/live-answer UX as not yet safe for demo sign-off.

## Why
- The new QA regressions confirm the intended split between **visibility** and **answerability** in `rep-phone.js`: pending can be shown early, but Accept is only enabled once a real ACS `incomingCall` object exists.
- However, `live-transcript.js` `resync()` still promotes any `/api/calls/active` payload with a `callId` into `onCallPending(...)`, without checking `acceptAvailable`, `repAccepted`, or `state`.
- That means a reload/reconnect can still present a misleading pending/Accept affordance for an already-accepted live call, and a live frontend pointed at mock/scripted state can imply a live answer path that does not exist.

## Evidence
- Added QA regressions in `tests/CallCenterTranscription.Tests/RepCallControlTests.cs`:
  - `Frontend_AcceptOffer_ShowsOnPendingCall_ButOnlyEnablesAfterIncomingInvite`
  - `Frontend_PendingCall_KeepsSpeechBadgeConnectedWhileShowingAcceptOffer`
  - `Frontend_Resync_GatesPendingOfferOnAnswerableActiveCallState`
  - `Frontend_Resync_DoesNotReopenAcceptedOrMockCallsAsPending`
- Test results:
  - `dotnet test tests/CallCenterTranscription.Tests/CallCenterTranscription.Tests.csproj --nologo --no-restore --filter "RepCallControlTests"`
  - **FAIL**: `Frontend_Resync_GatesPendingOfferOnAnswerableActiveCallState`
  - **FAIL**: `Frontend_Resync_DoesNotReopenAcceptedOrMockCallsAsPending`

## Required revision
A different agent should update `live-transcript.js` resync/bootstrap logic to:
1. treat `acceptAvailable` as the authoritative signal for reopening the pending Accept UI,
2. explicitly honor accepted/repAccepted state without dispatching `rep.callPending`, and
3. ignore mock/scripted active-call payloads that are not truly answerable.

# Yzak — Accept answerability re-review
...
## Decision
Approve the revised accept-answer/resync path for this gate.

## Why
- `live-transcript.js` `resync()` now reopens pending only when `/api/calls/active` reports `acceptAvailable: true`.
- Accepted-call recovery now requires `/api/session/current-state` to confirm the same `callId`, a non-mock snapshot, and an `accepted` / `active` / `live` state before `onCallAccepted(...)` runs.
- This closes the prior bug where any active `callId` could reopen a misleading pending/Accept affordance after reload/reconnect.

## Evidence
- Focused regression pass:
  - `dotnet test tests/CallCenterTranscription.Tests/CallCenterTranscription.Tests.csproj --nologo --no-restore --filter "RepCallControlTests|ApiHost_LiveMode_ExposesPendingCallStateBeforeAnyTranscript|ApiHost_LiveMode_ExposesAcceptedCallStateWithoutReopeningAccept|ApiHost_LiveMode_ExposesAcceptedCallStateEvenIfPendingWasMissed|ApiHost_MockMode_DoesNotExposeSyntheticPendingAcceptState|EmitCallPendingAsync_EmitsPendingEvent|IsCurrentActiveCall_MatchesOnlyExactActiveCallId"`
  - Result: **32 total, 30 passed, 2 skipped, 0 failed**
- Broad regression pass:
  - `dotnet test CallCenterTranscription.sln --nologo --no-restore`
  - Result: **133 total, 131 passed, 2 skipped, 0 failed**
...
- **Source:** `.squad/decisions/inbox/copilot-directive-2026-06-12T10-06-22.md`

### 2026-06-12T10:06:22.404-04:00: User directive
**By:** Jason (via Copilot)
**What:** Use only English for the demo.
**Why:** User request — captured for team memory

- **Source:** `.squad/decisions/inbox/copilot-directive-2026-06-12T10-04-08.md`

### 2026-06-12T10:04:08.603-04:00: User directive
**By:** Jason (via Copilot)
**What:** Assume the demo can be run using our Azure resources.
**Why:** User request — captured for team memory
